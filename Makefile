...
all:
	...
	# 説明: WIP: チャンネルの動画情報を取得するスクリプト
	# 説明: チャンネルの動画情報から dataset/sources.yml を作成するスクリプト
	ruby scripts/search_list_to_yaml/main.rb
	ruby scripts/search_list_to_yaml/merge_sources_yaml.rb
	# 説明: チャンネルの動画情報から動画一覧ページのデータを作成するスクリプト
	ruby scripts/search_list_to_yaml/videolist_to_page.rb
	# 説明: wav ファイルから入力用の yaml dataset/sounds 以下に作成するスクリプト
